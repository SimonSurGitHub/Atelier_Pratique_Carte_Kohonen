...
class Neuron:
  ...
  def compute(self,x):
    ...
    '''
    # TODO
    self.y = numpy.sum(numpy.sqrt( (x - self.weights)**2 ))
    # self.y = None
  ...
  def learn(self,eta,sigma,posxbmu,posybmu,x):
    ...
    # d2 = math.sqrt( (posxbmu - self.posx)**2 + (posybmu - self.posy)**2 )
    d2 = math.sqrt( (posxbmu - self.posx)**2 + (posybmu - self.posy)**2 )

    v = math.exp( -(d2**2) / (2*sigma**2) )
...
class SOM:
  ...
  def scatter_plot(self,interactive=False):
    ...
    plt.xlim(-1,1)
    plt.ylim(-1,1)
    liste = numpy.concatenate(self.weightsmap, axis=0 )
    hull = ConvexHull(liste)
    # vertices = hull.vertices.tolist() + [hull.vertices[0]]
      
    # perimeter = numpy.sum([euclidean(x, y) for x, y in zip(liste[vertices], liste[vertices][1:])])
    #print(perimeter)
    # Affichage du titre de la figure
    plt.suptitle('Poids dans l\'espace d\'entree\n' + "Aire du nuage convexe : " + str(round(hull.volume,2)))
    #plt.suptitle()
...
if __name__ == '__main__':
  ...
  # Ensemble de données perso 1
  # nsamples = 1000
  # samp1 = []
  # for i in range (0,1500) :
  #   samp1.append(math.sin(i))
  # samp2 = numpy.ogrid[0.7:-0.7:500j]
  # samp2[::2] = 0.7
  # samples = numpy.concatenate((samp1,samp2)).reshape(nsamples,2,1)
  # Ensemble de données perso 2
  # samp1 = numpy.ogrid[-0.99:0.99:1000j]
  # samp1[::2] = -0.
  # samp2 = numpy.ogrid[-0.99:0.99:1000j]
  # samp2[1::2] = -0.
  # samples = numpy.concatenate((samp1,samp2)).reshape(nsamples,2,1)
